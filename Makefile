...
.PHONY: help init build test deploy clean status health gen-sbom sign verify contract-test tri-run merge anchor promote hooks-install orchestrator-sim perf-001 test-perf-001
...
init:
	...
	@mkdir -p $(SANDBOX)/inputs $(SANDBOX)/outputs $(SANDBOX)/tri-sandbox/{A,B,C} $(SANDBOX)/parent/model-D
	@echo "📦 Checking Rust dependencies..."
	@command -v cargo >/dev/null 2>&1 && cargo check || echo "[init] cargo not found; skipping Rust checks (Python-only mode)"
	@echo "🐍 Setting up Python environment..."
	@cd unified_agents && $(PY) -m pip install --upgrade pip --quiet || echo "[init] pip upgrade skipped"
	@echo "🔧 Setting up Git hooks..."
	@cp hooks/pre-push .git/hooks/pre-push 2>/dev/null || echo "[init] No pre-push hook to install"
...
test: test-core test-integration test-perf-001
...
test-perf-001:
	@echo "🧪 Running PERF-001 Subject Test (Python Orchestrator)"
	$(PY) TEST/perf/test_perf_001.py

perf-001: test-perf-001
	@echo "[SUBJECT] Executing PERF-001: Implement performance optimization system"
	$(PY) $(TOOLS)/performance_optimizer.py
	@echo "🧪 Running integration tests..."
	cd tests/integration && cargo test
	@echo "✅ Integration tests passed!"

# SLA-001: Achieve SLA targets
sla-001: test-slo-001
	@echo "[SUBJECT] Executing SLA-001: Achieve SLA targets"
	@echo "✅ SLA-001 execution complete. See TEST/slo/slo_check.log for results."

test-slo-001:
	@echo "🧪 Running SLA-001 Subject Test (SLO Validation Script)"
	@mkdir -p TEST/slo
	@pip install pyyaml > /dev/null 2>&1 || echo "pyyaml already installed or failed to install"
	@$(PY) TEST/slo/test_slo_check.py
	@echo "✅ SLA-001 SLO validation test passed."

# PERF-RATE: Rate-limiting and throttling strategy
perf-rate: test-perf-rate
	@echo "[SUBJECT] Executing PERF-RATE: Rate-limiting and throttling strategy"
	@echo "✅ PERF-RATE execution complete. See TEST/perf/rate/rate_limit_test.log for results."

test-perf-rate:
	@echo "🧪 Running PERF-RATE Subject Test (Rate Limit Simulation)"
	@mkdir -p TEST/perf/rate
	@pip install pyyaml > /dev/null 2>&1 || echo "pyyaml already installed or failed to install"
	@$(PY) TEST/performance/rate_limit/test_rate_limit.py
	@echo "✅ PERF-RATE Rate Limit Simulation test passed."
