...
impl CommunicationManager {
    ...
    async fn resolve_broadcast_scope(&self, scope: &BroadcastScope) -> Result<Vec<AgentId>> {
        ...
        let agents = match scope {
            BroadcastScope::All => registry
                .all_agents()
                .into_iter()
                .map(|meta| meta.id)
                .collect(),
            BroadcastScope::Layer(layer) => registry.find_by_layer(layer),
            BroadcastScope::Role(role) => registry.find_by_role(role),
        };
    ...
    fn get_target_agent(&self, message: &AgentMessage) -> Result<AgentId> {
        match message {
            ...
            AgentMessage::Registration { .. } => {
                ...
                Err(anyhow::anyhow!(
                    "Registration messages should use broadcast"
                ))
            }
    ...
    pub async fn start_background_tasks(&self) -> Result<()> {
        ...
        let heartbeat_manager =
            HeartbeatManager::new(self.registry.clone(), self.config.heartbeat_interval);
        ...
        loop {
            ...
            for (message_id, pending) in tracker.pending_deliveries.iter_mut() {
                if pending.attempts < self.max_retries {
                    if (now - pending.last_attempt).num_seconds()
                        >= self.retry_backoff.as_secs() as i64
                    {
...
impl MetricsCollector {
    ...
    async fn run(&self) -> Result<()> {
        ...
        loop {
            ...
            tracing::debug!(
                "Communication metrics updated: throughput = {:.2} msg/s",
                metrics.throughput
            );
        }
    }
