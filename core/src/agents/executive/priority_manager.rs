...
impl PriorityManager {
    ...
    pub async fn calculate_priority(&self, task: &Task) -> Result<f64> {
        ...
        for factor in &priority_engine.calculation_factors {
            ...
            let factor_value = self
                .calculate_factor_value(&factor.factor_type, task)
                .await?;
    ...
    pub async fn get_next_task(&self) -> Result<Option<Task>> {
        ...
        if let Some(prioritized_task) = scheduler.task_queue.pop() {
            ...
            tracing::debug!(
                "Retrieved task {} with priority {:.2}",
                task.id,
    ...
    pub async fn escalate_priority(&self, task_id: Uuid, escalation_factor: f64) -> Result<()> {
        ...
        if let Some(assignment) = priority_engine.priority_assignments.get_mut(&task_id) {
            if !assignment.locked {
                let new_priority =
                    (assignment.current_priority * (1.0 + escalation_factor)).clamp(0.0, 100.0);
                ...
                tracing::info!(
                    "Escalated priority for task {} to {:.2}",
                    task_id,
                    new_priority
                );
            } else {
                tracing::warn!(
                    "Cannot escalate locked priority assignment for task {}",
                    task_id
                );
            }
        }
    ...
    pub async fn apply_policy(&self, policy_id: &str) -> Result<()> {
        ...
        if let Some(policy) = policy_manager
            .active_policies
            .iter()
    ...
    async fn calculate_factor_value(
        &self,
    ...
    async fn check_policy_conditions(
        &self,
        _conditions: &[PolicyCondition],
    ) -> Result<Vec<String>> {
        ...
        match task.name.as_str() {
            ...
            "escalate-priority" => {
                ...
                Ok(TaskResult {
                    ...
                    status: TaskStatus::Completed,
                    output_data: Some(
                        serde_json::json!({"escalated": true, "factor": escalation_factor}),
                    ),
                    error_message: None,
                    completed_at: chrono::Utc::now(),
                })
...
impl PriorityManager {
    /// Initialize priority factors
    async fn initialize_priority_factors(
        &self,
        priority_engine: &mut PriorityEngine,
    ) -> Result<()> {
        let factors = vec![
            PriorityFactor {
        ...
        tracing::info!(
            "Initialized {} priority factors",
            priority_engine.calculation_factors.len()
        );
        Ok(())
    }
    ...
    async fn initialize_default_policies(
        &self,
        _policy_manager: &mut PriorityPolicyManager,
    ) -> Result<()> {
    ...
    async fn run_priority_calculation(priority_engine: Arc<RwLock<PriorityEngine>>) -> Result<()> {
        ...
        for assignment in priority_engine.priority_assignments.values_mut() {
            if !assignment.locked {
                ...
                assignment.current_priority =
                    (assignment.base_priority + age_bonus).clamp(0.0, 100.0);
        ...
        let avg_priority: f64 = priority_engine
            .priority_assignments
            .values()
    ...
    async fn run_sla_monitoring(sla_monitor: Arc<RwLock<SLAMonitor>>) -> Result<()> {
        ...
        // Update SLA metrics
        let compliant_count = sla_monitor
            .compliance_tracking
            .values()
        ...
        sla_monitor.metrics.avg_compliance = sla_monitor
            .compliance_tracking
            .values()
            .map(|c| c.compliance_percentage)
            .sum::<f64>()
            / sla_monitor.compliance_tracking.len().max(1) as f64;
