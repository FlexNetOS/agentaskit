...
pub trait AgentLifecycleProtocol {
    ...
    async fn update_agent_metadata(&self, agent_id: AgentId, metadata: AgentMetadata)
        -> Result<()>;
...
pub trait DeploymentManagementProtocol {
    ...
    async fn update_deployment(
        &self,
        agent_id: AgentId,
        manifest_entry: DeploymentManifestEntry,
    ) -> Result<()>;
...
pub trait MetricsCollectionProtocol {
    ...
    async fn get_system_metrics(
        &self,
        time_range: TimeRange,
    ) -> Result<HashMap<AgentId, Vec<SystemMetrics>>>;
